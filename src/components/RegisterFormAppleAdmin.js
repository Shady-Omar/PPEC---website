...
function RegFormAdminApple() {
  ...
  const submit = async (e) => {
    ...
    var numRegex = /1?[\s-]?\(?(\d{3})\)?[\s-]?\d{3}[\s-]?\d{4}/;
        ...
        if (adminFirst === null || adminFirst === "" || adminLast === null || adminLast === "" || !adminEmail.match(emailRegex) || duplicate(duplicateEmail) === true || !adminNum.match(numRegex) || adminJob === null || adminJob === "") {
...
export default RegFormAdminApple;